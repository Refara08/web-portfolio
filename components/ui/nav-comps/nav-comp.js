...
const NavComp = (props) => {
  ...
  return (
    <div
      ref={navChildRef}
      className={`flex flex-wrap justify-between items-center gap-4 px-8 pt-8 sm:pt-12 pb-4 bg-primary text-txt-prime sticky top-0 z-20
      `}
    >
      <div className="pl-4">
        <Link href="/">
      ...
      <nav>
        <ul className="flex justify-center items-center gap-4">
          {navItems.map((item, index) => (
            <li className="block nav-child" key={index}>
